...
/*   Updated: 2025/10/13 15:42:13 by fraalexa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void checknsend_bit(int pid, char c, int nbit)
{
	...
	{
		write(1, "unable to sendig signal", 30);
        exit(2);
	}
    else if (!(c & (1 << nbit)) && kill(pid, SIGUSR2) == -1)
	{
		write(1, "unable to sendig signal", 24);
        exit(2);
	}
...
void	ack_handler(int ack, siginfo_t *info, void *context)
{
	(void)info;
	(void)context;
	if (ack == SIGUSR1)
